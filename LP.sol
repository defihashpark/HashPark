...
contract LP   {
    ...
    function deposite(address token, uint256 amount, address parent) public payable reqEnable /*lockAddr(msg.sender)*/   {
        ...
        _lp[msg.sender][token].hpcPrice = price;
    ...
    function getTokenUsdtPriceFromWBNBPool(
        ...
        // scale 1e18
        return usdtAmt.mul(1e18).mul(wbnbAmt1).div(wbnbAmt2.mul(tokenAmt));
    }
        ...
        returns (uint256)
    {
        ...
        if (_pool[token].poolType == ePoolType.usdt) {
            return getTokenUsdtPriceFromUsdtPool(_pool[token].pancake);
        } else {
            return getTokenUsdtPriceFromWBNBPool(token, _pool[token].pancake);
        }
    }
